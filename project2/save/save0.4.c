...
int find_list_2(student *frst, char name[]);
void print_list_2(student *ptr);
unsigned long hash(char *str);
int add_hash(book *ptr,student *hash,student *ne);
...
//desmesi kai arxikopoisi tou pinaka
    ...
    hash = (student *) malloc(H*sizeof(student));
...
int add_list_2(student *pr,student *ne){
    struct student *nxt,*prv,*neo;

    prv = (struct student *) pr;
...
int add_hash(book *ptr ,student *hash,student *ne){
    unsigned long int k;
    int i,j,new,pply;
    student *next,*neo,new_hash;
    
    k = hash(ne->name)/ptr->max_hash;
    next=hash[i];
    ...
    {//gia ayjisi toy pinaka
        new_hash = (student *) malloc((ptr->max_hash * 2)*sizeof(student)
        if(new_hash == NULL){
            printf("NO space");
            return 0;
        }
