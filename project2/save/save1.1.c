...
int add_list_2(struct student *pr,struct student *ne);
int rmv_list_2(struct student *dell); // not used free 
int find_list_2(struct student *frst, char name[],int metritis);
void print_list_2(struct student *ptr);
unsigned long hash(char *str);
int add_hash(book *ptr,struct table *hasht,struct student *ne);
int rmv_hash(book *ptr,struct table *hasht,struct student *del);
...
int find_by_name (book *ptr, struct table *hash, char name[]);

// function project 1
void clear (book *ptr,struct table *hasht);
int  add (book *ptr,struct table *hasht,const long unsigned int AEM,const char name[],const short unsigned int mathima,const int K);
...
void print (const book *ptr,struct table *hasht);
int rmv (book *ptr,struct table *hasht,const long unsigned int AEM,const int K);
void sort (book *ptr);
void quit (book *ptr,struct table *hasht);

int main(int argc ,char *argv[]){
    int M=0,K=0,H=0,i=0,maxs;
    book univer,*books;
    struct student *ptr;
    struct table *ted, *head;
    long unsigned int AEM;
    char NAME[64];
    char menu;
...
//desmesi kai arxikopoisi tou pinaka
    if (M>0)
    {
        ptr = (struct student *) malloc(M*sizeof(struct student));
        univer.students = ptr;
        books = &univer;
        for(i=0;i<M;i++){
    ...
    ted = ( struct table *) malloc(H*sizeof(struct table));   
    head = ted;
    for ( i = 0; i < H; i++)
    {
        head->HEAD = NULL;
    ...
    while (1){
        ...
        switch (menu)
        {
            case 'a':
                ...
                add(books,ted,AEM,NAME,MATHIMA,K);
                break;
...
int  add (book *ptr,struct table *hasht,const long unsigned int AEM, const char name[],const short unsigned int mathima,const int K){
    int i=0,j,*p,k;
    struct student *tptr;// gia na min xaso ta idei apothikeymena

    p=&i;
    if(ptr->tax == -1){
        tptr = (struct student *) malloc(K*sizeof(struct student));
        if(tptr == NULL){
    ...
    {//auxisi pinaka kai elenxos ean petixe
        k=ptr->size;
        tptr =(struct student*) realloc (ptr->students,(ptr->size+K)*sizeof(struct student) );
        if (tptr == NULL){
            printf("\nA-NOK %lu, %d %d\n",AEM,ptr->full,ptr->size);
            return 0;
        }
...
void print (const book *ptr,struct table *hasht){
    ...
    printf("\n#\n%d %d %d %d",ptr->max_hash,ptr->full,LF,max);
    max=ptr->max_hash;
    has = hasht;
    for (i = 0; i < max; i++)
    { 
        printf("\n%d %d",i,has->megethos);
        ted = has->HEAD;
        for ( j = 0; j < has->megethos; j++)
        {
            printf("\n%lu %s %hu\n",ted->aem,ted->name,ted->mathima);
            ted = ted->next;
        } 
        has++;
        printf("\n");p
...
int rmv (book *ptr,struct table *hasht,const long unsigned int AEM,const int K){
    ...
    if (i!= -1){
        ...
        if (max == K)
        {
            tptr=(struct student *) realloc(ptr->students,(k - K)*sizeof(struct student));
            if (tptr == NULL)
            {
                printf("\nR-NOK %lu, %d %d\n",AEM,ptr->full,ptr->size);
                return 0;
...
int add_list_2(struct student *prt,struct student *neo){
    struct student *nxt;
...
int rmv_list_2(struct student *del){
    struct student *prv, *nxt;
...
int add_hash(book *ptr ,struct table *hasht,struct student *neo){
    unsigned long int k;
    int i,new, LF;
    struct student *next, *head, *ne ;
    struct table *new_hash,*ted;

    k = hash(neo->name)/ptr->max_hash;
    new_hash =  hasht + k;
    if (new_hash->HEAD == NULL)
    {
        new_hash->HEAD = neo;
        new_hash->megethos = 1;
        neo->next = neo->prev = neo;
        ptr->ply++;
        return 0;
    }
    ...
    while(1)
    {
        ...
        if (strcmp(next->name,neo->name)>0)
        {
            next=next->next;
            if (next == head)
            {
    ...
    {//gia ayjisi toy pinaka
        new_hash = (struct table *) malloc((ptr->max_hash * 2)*sizeof(struct table));
        if(new_hash == NULL){
            printf("NO space");
            return 0;
        }
...
int find_by_name(book *ptr,struct table *hasht, char name[]){
    unsigned long int k;
    int i ,metritis=0;
    ...
    head = ted->HEAD;
    metritis++;
    i = find_list_2(head,name,metritis);
